...
源码位于 `src/` 目录，构建产物是 `package/alien-code`（Bun 独立二进制）。**任何修改 src/ 下文件的操作之后，必须立即运行构建命令**：
...
构建入口是 `build.ts`，采用两阶段构建：先用 Bun.build() API 将 TS 项目打包为中间 ESM 文件，再用 `bun build --compile` 编译为独立二进制。
...
Bun 处理 `void import('./devtools.js')` 时会生成无效语法 `Promise.resolve().then(() => )`，`build.ts` 在中间 JS 文件上用正则自动修复为 `Promise.resolve()`，然后再编译为二进制。
